...
const CuratorList = () => {
  ...
  const renderFocusedItem = ({ item }) => <UserCard userAvatar={item?.profileImage ? `${profileAssetUrl}${item?.profileImage}`  : defaultAvatar} userName={item.name} score={item.count} menu={false} id={item?._id} isFollowed={item?.isFollowed}/>;
